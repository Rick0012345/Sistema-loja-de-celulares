        ...
        <table className="w-full border-collapse text-left">
          <thead>
            <tr className="border-b border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-slate-950">
              <th className="p-3.5 font-semibold text-slate-600 dark:text-slate-300">Servico / OS</th>
              <th className="p-3.5 font-semibold text-slate-600 dark:text-slate-300">Valor Cobrado</th>
              <th className="p-3.5 font-semibold text-slate-600 dark:text-slate-300">Custo Pecas</th>
              <th className="p-3.5 font-semibold text-slate-600 dark:text-slate-300">Lucro Bruto</th>
              <th className="p-3.5 font-semibold text-slate-600 dark:text-slate-300">Margem</th>
            </tr>
          </thead>
          <tbody>
            {profitData.length === 0 && (
            ...
            {profitData.map((item) => (
              <tr key={item.id} className="border-b border-slate-100 transition-colors hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-800/70">
                ...
                <td className="p-3.5 font-bold text-slate-900 dark:text-slate-100">{formatCurrency(item.totalPrice)}</td>
                <td className="p-3.5 font-medium text-rose-600">-{formatCurrency(item.partsCost)}</td>
                <td className="p-3.5 font-black text-emerald-600">{formatCurrency(item.profit)}</td>
                <td className="p-3.5">
