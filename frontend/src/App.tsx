    ...
    <div className="flex h-screen bg-slate-50 font-sans text-slate-900 transition-colors dark:bg-slate-950 dark:text-slate-100">
      ...
      >
        <div className="flex items-center gap-3 border-b border-slate-100 p-6 dark:border-slate-800">
          ...
          {isSidebarOpen && <h1 className="text-xl font-bold tracking-tight">ConsertaSmart</h1>}
        </div>
        ...
        <nav className="flex-1 space-y-2 p-4">
          {navItems.map((item) => (
            <button
              ...
              className={cn(
                'group flex w-full items-center gap-3 rounded-xl p-3 transition-all',
                activeTab === item.id
      ...
      <main className="flex-1 overflow-y-auto p-8">
        <header className="mb-8 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h2 className="text-3xl font-bold tracking-tight">{currentLabel}</h2>
